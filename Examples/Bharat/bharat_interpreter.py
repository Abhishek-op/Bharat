...
def standard_library():
    ...
    env.update({
        ...
        'पो': lambda base, exponent : pow(base, exponent),
...
class Process:
    """
    The main process the executes Bharat Abstract Syntax Tree
    """
    def __init__(self, tree, filename="?", env={}):
        ...
        self.depth = 0
        self.types  =  { 'इन्ट': int, 'फ्लोट': float, 'स्ट्रिग': str, 'बूल': bool, 'लिस्ट': list, 'डिक्ट': dict }
        self.rtypes = { int: 'इन्ट', float: 'फ्लोट', str: 'स्ट्रिग', bool: 'बूल', list: 'लिस्ट', dict: 'डिक्ट' }
    ...
    def evaluate(self, parsed):
        ...
        if type(parsed) != tuple:
            ...
            if action == 'class_func_call':
            	...
            	print(self.evaluate(parsed[1]))
            elif action == 'raw_input':
            	input()
            	
            elif action == 'class':
                name = parsed[1]
                if name in self.env:
                    raise NameError('रिडिफाइन नही कर सकते वैरिएबल \'%s\'' % name)
                    return None
                ...
                if struct_definition is None:
                    raise UnboundLocalError('स्टरक्ट %s अनडिफाइन है' % name)
                    return None
                ...
                for i in range(len(fields)):
                    if i < len(values):
                        if type(values[i]) != self.types[fields[i][1]]:
                            raise ValueError("फील्ड के लिए टाइप '{}' होना चाहिए '{}' लेकिन  '{}' मिला ".format(fields[i][0], fields[i][1], self.rtypes[type(values[i])]))
                ...
                if isinstance(func, OClass):
                    ...
                    if type(func) == type(lambda x: x):
                        ...
                        raise ValueError('\'%s\' फंसन नही है' % parsed[1])
                ...
                if type(parsed[1]) is not tuple:
                    if parsed[1] not in self.env:
                        raise UnboundLocalError(' असाइन  नही कर सकते अनडिफाइन वैरिएबल \'%s\'' %
                              parsed[1])
                    result = self.evaluate(parsed[2])
                    var = self.env.find(parsed[1])
                    if type(result) != var.type:
                        raise ValueError("वैरिएबल के लिए टाइप '{}' होना चाहिए '{}' लेकिन  '{}' मिला ".format(parsed[1], self.rtypes[var.type], self.rtypes[type(result)]))
            	...
            	for i in range(self.evaluate(parsed[1]), self.evaluate(parsed[2])):
                ...
                if index > len(var):
                    raise IndexError('इन्डेक्स आउट ऑफ बाउन्ड इरर')
                    return None
                elif type(index) != int:
                    raise IndexError('लिस्ट के इन्डेक्स इन्ट होने चाहिए')
                    return None
                return var[index]
            elif action == '+':
                result = self.evaluate(parsed[1])
...
class Env(dict):
    ...
    def find(self, name):
        ...
        raise UnboundLocalError("{} अनडिफाइन्ड है".format(name))


class Function(object):
    """
    Function object for Bharat Functions and annoymous functions (lambdas)
    """
    ...
    def __call__(self, *args):
        ...
        for i in range(len(self.params)):
            if type(args[i]) != self.process.types[self.params[i][1]]:
                raise TypeError("पैरामीटर के लिए टाइप '{}' होना चाहिए '{}' लेकिन  '{}' मिला .".format(self.params[i][0], self.params[i][1], self.process.rtypes[type(args[i])]))
